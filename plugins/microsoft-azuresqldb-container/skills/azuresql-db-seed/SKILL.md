...
description: >-
  ...
  "write a seed script", or "bulk load a CSV". This is the Azure SQL engine
  (EngineEdition 5), not the mssql/server SQL Server image. Distinct from
  azuresql-db-scaffold (which does a single seed.sql step while bootstrapping an
  app) and azuresql-db-import (which loads a .bacpac). Reach for this whenever an
...
insert books that reference those author ids. Open [references/seed-snippets.md](references/seed-snippets.md) when
you need copyable T-SQL for a model with more than two tables.
...
join against to fan out rows in a single statement. Open [references/seed-snippets.md](references/seed-snippets.md) when
you want the runnable version, which generates 1000 rows. Wrap large inserts in an explicit
transaction so a mid-load failure rolls back cleanly.

## Step 4: pick your recipe

Open [references/seed-snippets.md](references/seed-snippets.md) once you know your data source and
stack and need the matching runnable recipe:
...
- **Positive verification:** Run the seed twice and require the requested stable counts, no orphaned rows, and unchanged protected rows.
- **Cleanup verification:** Remove seed-owned rows or drop the disposable database, then remove temporary CSV and script copies.
