...
description: >-
  ...
  Use this INSTEAD of the Testcontainers MsSql preset (mcr.microsoft.com/mssql/server),
  because this is the Azure SQL engine (EngineEdition 5). For wiring the engine
  into a CI pipeline via service containers or workflow YAML instead, use
  azuresql-db-ci.
---
...
- **Positive verification:** Run the selected test suite and require isolated databases, exact expected rows, and zero test failures.
- **Cleanup verification:** Close connections and remove every test-owned container, network, database, and temporary file.
...
- [references/testcontainers-snippets.md](references/testcontainers-snippets.md): open the recipe for your test stack when implementing the wait-for-ready strategy, `appdb` provisioning, mapped-port connection string, and disposal.
