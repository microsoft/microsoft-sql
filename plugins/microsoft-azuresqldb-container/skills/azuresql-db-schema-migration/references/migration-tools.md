...
- Generate an idempotent script to review before applying:
  ...
  `SQLCMDPASSWORD="$MSSQL_SA_PASSWORD" sqlcmd -S "localhost,$HOST_PORT" -U sa -C -b -d appdb -i migrate.sql`.
...
Use SqlPackage response-file support through shell process substitution so
Publish and Extract receive the password without placing it in process
arguments or a persistent file.
