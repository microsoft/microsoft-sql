...
T-SQL (generated nonzero vector, for quick checks via sqlcmd):

```sql
DECLARE @embedding NVARCHAR(MAX) =
  N'[1.0,' + REPLICATE(N'0.0,', 766) + N'0.0]';
INSERT INTO docs (content, embedding)
VALUES (N'hello', CAST(@embedding AS VECTOR(768)));
```
...
Executable nearest-neighbor example using a generated nonzero vector:

```sql
DECLARE @query_vector NVARCHAR(MAX) =
  N'[1.0,' + REPLICATE(N'0.0,', 766) + N'0.0]';
SELECT TOP (5) content,
       VECTOR_DISTANCE('cosine', embedding, CAST(@query_vector AS VECTOR(768))) AS distance
FROM docs
...
```bash
docker exec -i -e SQLCMDPASSWORD sqldb /opt/mssql-tools18/bin/sqlcmd \
