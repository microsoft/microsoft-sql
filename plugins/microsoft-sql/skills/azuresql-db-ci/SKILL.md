  ...
  test:
    ...
    services:
      sqldb:
        ...
        options: >-
          --health-cmd "/opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b -l 2 -Q \"SELECT 1\""
          --health-interval 5s
          --health-timeout 3s
          --health-retries 20
    ...
    steps:
      ...
      - name: Seed appdb
        run: |
          ...
          docker exec "$CID" /opt/mssql-tools18/bin/sqlcmd \
            -S localhost -U sa -C -b -d appdb -i /tmp/seed.sql
...
- **Azure Pipelines:** there is no native `services:`. Sign in and run the **canonical start
  recipe** below (it provisions appdb in the ready-wait loop) in one Bash task:
  ```yaml
  - bash: |
      printf '%s' "$ACR_PASSWORD" | docker login sqldbpreview-dpgaeqhmgphzd4bk.azurecr.io --username "$ACR_USERNAME" --password-stdin
      # Run the canonical start recipe below in this task.
    env:
      ACR_USERNAME: $(ACR_USERNAME)
      ACR_PASSWORD: $(ACR_PASSWORD)
      MSSQL_SA_PASSWORD: $(MSSQL_SA_PASSWORD)
  ```
  Then run tests with `SQL_CONNECTION_STRING` pointed at appdb.
- **GitLab CI:** use a `services:` entry for the image, set `DOCKER_AUTH_CONFIG` for the private
  registry, set `ACCEPT_EULA`, and map a masked `MSSQL_SA_PASSWORD_UI` variable to both
  `MSSQL_SA_PASSWORD` and `SQLCMDPASSWORD`, then provision appdb in a `before_script` with `docker
  exec` (or a sqlcmd container) before tests.
...
(
set +x
: "${MSSQL_SA_PASSWORD:?Set MSSQL_SA_PASSWORD in the CI secret store and map it into this step}"
...
- **Positive verification:** Require the workflow test command to exit zero and the readiness query to return the expected engine and application database.
- **Cleanup verification:** Require the service container, temporary database files, and workflow-owned network to be removed after the job.
