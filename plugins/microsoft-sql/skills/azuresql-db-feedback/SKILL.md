...
- **Positive verification:** A CLI submission is complete only when it prints the created issue URL; a prefilled URL is complete only when it opens the expected host and template.
- **Cleanup verification:** Delete any local draft body or attachment copy that was created solely for submission after the URL is verified.
