...
description: >-
  ...
  keeps dropping", "connections time out under load", "add backoff", or "make
  the DB layer resilient". This is the Azure SQL engine (EngineEdition 5), not
  the mssql/server SQL Server image. Reach for this whenever hardening a
...
- Required env: `ACCEPT_EULA=Y` and a complex, caller-supplied `MSSQL_SA_PASSWORD`. The
  engine listens on 1433.
...
```bash
: "${MSSQL_SA_PASSWORD:?Set MSSQL_SA_PASSWORD to a unique local administrator password}"
...
until docker exec sqldb sh -c \
  'SQLCMDPASSWORD="$MSSQL_SA_PASSWORD" exec /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b -l 2 -Q "$1"' sh \
  "IF DB_ID('appdb') IS NULL CREATE DATABASE appdb;" \
  >/dev/null 2>&1; do sleep 2; done
echo "ready on localhost,$HOST_PORT"
```

Use this non-executable connection-string template with the selected `HOST_PORT`:
...
- **Positive verification:** Exercise a genuine transient connection fault and confirm the operation succeeds within the configured attempt and pool bounds.
- **Cleanup verification:** Dispose every cursor and connection, then confirm pooled/active connection counts return to their pre-test level.
...
- [references/retry-snippets.md](references/retry-snippets.md): open the section for your stack when implementing pooling and transient-only retry for .NET, Node, or Python.
