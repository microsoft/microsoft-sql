...
export SQLCMDPASSWORD="${MSSQL_SA_PASSWORD:?Set MSSQL_SA_PASSWORD to a private password}"
...
docker run -d --name "$CONTAINER" "${PLATFORM[@]}" \
  -e "ACCEPT_EULA=Y" \
  -e MSSQL_SA_PASSWORD -e SQLCMDPASSWORD \
...
ATTEMPTS=0
MAX_ATTEMPTS=60
until docker exec "$CONTAINER" "$SQLCMD" -S localhost -U sa -C -b -l 2 \
...
ENGINE_EDITION="$(docker exec "$CONTAINER" "$SQLCMD" -S localhost -U sa -C -b -l 2 \
  -h -1 -W -Q "SET NOCOUNT ON; SELECT CAST(SERVERPROPERTY('EngineEdition') AS INT);" | tr -d '[:space:]')"
EDITION="$(docker exec "$CONTAINER" "$SQLCMD" -S localhost -U sa -C -b -l 2 \
...
docker exec "$CONTAINER" "$SQLCMD" -S localhost -U sa -C -b -l 2 \
