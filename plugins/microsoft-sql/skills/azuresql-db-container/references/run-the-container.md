...
podman run -d --name sqldb --platform linux/amd64 -e "ACCEPT_EULA=Y" \
  -e MSSQL_SA_PASSWORD -p "127.0.0.1:${HOST_PORT:-1433}:1433" \
...
services:
  sqldb:
    ...
    environment:
      ACCEPT_EULA: "Y"
      MSSQL_SA_PASSWORD: "${MSSQL_SA_PASSWORD:?Set and export MSSQL_SA_PASSWORD}"
      SQLCMDPASSWORD: "${MSSQL_SA_PASSWORD:?Set and export MSSQL_SA_PASSWORD}"
    ports:
      - "127.0.0.1:${HOST_PORT:-1433}:1433"
    volumes:
...
```bash
docker compose exec sqldb /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b \
  -Q "IF DB_ID('appdb') IS NULL CREATE DATABASE appdb;"
docker compose exec -T sqldb /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b \
