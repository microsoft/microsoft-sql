...
```bash
SQLCMDPASSWORD="${MSSQL_SA_PASSWORD:?Set and export MSSQL_SA_PASSWORD}" docker exec -e SQLCMDPASSWORD sqldb /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b \
  -d appdb -Q "SELECT DB_NAME() AS db, SERVERPROPERTY('EngineEdition') AS EngineEdition;"
```

## sqlcmd from the host

Use the host port chosen by the free-port loop. The
comma form `localhost,PORT` selects the port.

```bash
SQLCMDPASSWORD="${MSSQL_SA_PASSWORD:?Set and export MSSQL_SA_PASSWORD}" sqlcmd -S "localhost,${HOST_PORT:?Set HOST_PORT to the published host port}" -U sa -C -b \
...
| Node.js (mssql / tedious) | `{ server: "localhost", port: Number(process.env.HOST_PORT), user: "sa", password: process.env.MSSQL_SA_PASSWORD, database: "appdb", options: { trustServerCertificate: true } }` |
...
- Server: `localhost,<value of HOST_PORT>`
- Database: `appdb`
