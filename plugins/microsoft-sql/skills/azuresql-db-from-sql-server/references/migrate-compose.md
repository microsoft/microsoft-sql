...
Set `MSSQL_SA_PASSWORD` and the complete `SQL_CONNECTION_STRING` in the caller
environment using the same unique credential. The Before value targets
`Database=master`; the After value targets `Database=appdb`.
...
services:
  db:
    ...
    environment:
      ACCEPT_EULA: "Y"
      MSSQL_SA_PASSWORD: "${MSSQL_SA_PASSWORD:?Set MSSQL_SA_PASSWORD to a unique development secret}"
    ports:
      - "127.0.0.1:${HOST_PORT:?Set HOST_PORT to a free local port}:1433"
  app:
    build: ./app
    environment:
      SQL_CONNECTION_STRING: "${SQL_CONNECTION_STRING:?Set SQL_CONNECTION_STRING for the source database}"
    depends_on:
      - db
```
...
services:
  ...
  provision:
    ...
    environment:
      MSSQL_SA_PASSWORD: "${MSSQL_SA_PASSWORD:?Set MSSQL_SA_PASSWORD to a unique development secret}"
    entrypoint: ["/bin/sh", "-c", "SQLCMDPASSWORD=\"$$MSSQL_SA_PASSWORD\" exec /opt/mssql-tools18/bin/sqlcmd -S db,1433 -U sa -C -b -Q \"IF DB_ID('appdb') IS NULL CREATE DATABASE appdb;\""]
  app:
    build: ./app
    environment:
      SQL_CONNECTION_STRING: "${SQL_CONNECTION_STRING:?Set SQL_CONNECTION_STRING for appdb}"
    depends_on:
      provision:
...
5. **App connection string** re-pointed from `Database=master` to
   `Database=appdb`. Supply the complete value through `SQL_CONNECTION_STRING`
   from the caller environment and enable `TrustServerCertificate` for local use.
...
```bash
docker compose run --rm --entrypoint /bin/sh provision \
  -c 'SQLCMDPASSWORD="$MSSQL_SA_PASSWORD" exec /opt/mssql-tools18/bin/sqlcmd -S db,1433 -U sa -C -d appdb -i /seed.sql'
```
