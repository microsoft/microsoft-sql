...
services:
  ...
  sqldb:
    ...
    environment:
      ACCEPT_EULA: "Y"
      MSSQL_SA_PASSWORD: "${MSSQL_SA_PASSWORD:?Set MSSQL_SA_PASSWORD before running Docker Compose}"
      SQLCMDPASSWORD: "${MSSQL_SA_PASSWORD:?Set MSSQL_SA_PASSWORD before running Docker Compose}"
    ports:
      - "127.0.0.1:1433:1433"    # optional; only to reach it from this host
    healthcheck:
      # -b: a SQL error sets the exit code, so transient startup errors
      # (e.g. Msg 913) are retried, not masked. -l 2: short login timeout.
      test: ["CMD-SHELL", "/opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b -l 2 -Q \"SELECT 1\" || exit 1"]
      interval: 5s
      timeout: 10s
      retries: 30
      start_period: 30s
  ...
  sqldb-init:
    ...
    command:
      - >
        /opt/mssql-tools18/bin/sqlcmd -S sqldb -U sa -C -b
...
- **Positive verification:** Start the application and sidecar, require one successful database round trip, and verify the sidecar uses the declared endpoint.
- **Cleanup verification:** Stop and remove both services, their owned network, and temporary configuration files.
