...
```bash
until SQLCMDPASSWORD="${MSSQL_SA_PASSWORD:?Set and export MSSQL_SA_PASSWORD}" docker exec -e SQLCMDPASSWORD sqldb /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b -l 2 \
...
```bash
SQLCMDPASSWORD="${MSSQL_SA_PASSWORD:?Set and export MSSQL_SA_PASSWORD}" docker compose exec -e SQLCMDPASSWORD sqldb /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b \
