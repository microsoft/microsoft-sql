...
The plugin is designed for read-only questions about the database estate visible
to the signed-in Microsoft Entra user. It queries the Fabric Database Hub APIs,
follows required pagination, and reports missing permissions, incomplete metric
coverage, or partial results instead of treating missing data as healthy.
...
Database Hub does not expose Query Store, active sessions, query text, execution
plans, blocking, or database-level DMVs through this plugin. Requests such as
`Why is this query slow?` or `Show long-running queries in this database` require
an item-level SQL diagnostic capability and direct database access.
