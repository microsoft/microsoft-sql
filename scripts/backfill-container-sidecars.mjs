...
const AUTHORED = {
  ...
  'azuresql-db-feedback': {
    posture: ['read'],
    // The ONE skill with no engine surface. Its single probe is a curl on the
    // aka.ms redirect and no statement it makes could be contradicted by an
    // engine, so it is judged by the instrument that fits it. That declaration
    // is checked against its own SKILL.md, not believed: it fails if the body
    // names an engine message number, reads SERVERPROPERTY, carries T-SQL or
    // queries through sqlcmd.
    target: 'none',
    correction:
