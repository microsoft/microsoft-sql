...
**46 skills** are installed by the commands above. Expand a group to see what each one does.
...
Turns an Azure SQL Database connection error number or message into the cause and the fix, for every failure that happens before a credential is ever evaluated: 40615, 40914, 40532, 10928, 10929, 40501, 40544, 42101, 47072, 47073, and the pre-login, timeout and certificate failures that carry no SQL error number at all.
...
<details>
<summary><strong>Security and safe SQL</strong> (1)</summary>
...
Generates embeddings and chunks inside Azure SQL Database with CREATE EXTERNAL MODEL, AI_GENERATE_EMBEDDINGS, AI_GENERATE_CHUNKS and sp_invoke_external_rest_endpoint, including the database scoped credential naming rule, the permissions, and the endpoint allowlist that decides which hosts the engine is allowed to call at all.
...
A further 96 skills are planned. The full list, with the domain and intent of each, is in [catalog/catalog.json](catalog/catalog.json).
